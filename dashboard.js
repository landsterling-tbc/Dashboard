...
function makeDoughnut(id, dataMap, colorMap = {}) {
  ...
  const colors = entries.map(([k], i) => colorMap[k] || PALETTE[i % PALETTE.length]);

  /* plugin: رقم الإجمالي في منتصف الـ Doughnut */
  const centerPlugin = {
    id: "center_" + id,
    afterDraw(chart) {
  ...
  CHARTS[id] = new Chart(document.getElementById(id), {
    ...
    options: {
      ...
      plugins: {
        legend: {
          position: "bottom",
          labels: { font: { family: "Tajawal,sans-serif", size: 11, weight: "600" }, boxWidth: 10, boxHeight: 10, padding: 10, usePointStyle: true, pointStyle: "circle" }
        },
        tooltip: {
          callbacks: {
            label: (ctx) => ` ${ctx.label || "—"}: ${ctx.raw.toLocaleString("ar")} (${total ? ((ctx.raw / total) * 100).toFixed(1) : 0}%)`,
          },
        },
      },
...
function makeHBar(id, labels, values, colors, maxVal = null, fullLabels = null) {
  ...
  /* تنظيف الألوان — إزالة أي suffix (BB,DD,CC,88) */
  const cleanColors = (Array.isArray(colors) ? colors : [colors]).map(c =>
    typeof c === "string" ? c.replace(/[0-9A-F]{2}$/i, "") : (c || "#0b6b7e")
  );
  CHARTS[id] = new Chart(document.getElementById(id), {
    type: "bar",
    data: {
      ...
      datasets: [{
        label: "",
        data: values,
        backgroundColor: cleanColors.map(c => c + "BB"),
        borderColor:     cleanColors,
        borderWidth: 1.5,
        borderRadius: { topRight: 5, bottomRight: 5 },
        borderSkipped: "left",
        hoverBackgroundColor: cleanColors,
      }],
    },
    options: {
      indexAxis: "y",
...
function makeVBar(id, labels, datasets) {
  ...
  killChart(id);
  /* توحيد شكل الـ datasets */
  const styledDatasets = cleanDatasets.map((ds, i) => {
    const raw = Array.isArray(ds.backgroundColor) ? ds.backgroundColor[0] : ds.backgroundColor;
    const base = typeof raw === "string" ? raw.replace(/[0-9A-F]{2}$/i, "") : (PALETTE[i % PALETTE.length]);
    return {
      ...ds,
      backgroundColor: Array.isArray(ds.backgroundColor) ? ds.backgroundColor.map(c => typeof c === "string" ? c.replace(/[0-9A-F]{2}$/i,"") + "BB" : c) : base + "BB",
      borderColor:     Array.isArray(ds.borderColor)     ? ds.borderColor     : base,
      borderWidth: 1.5,
      borderRadius: 5,
      hoverBackgroundColor: Array.isArray(ds.backgroundColor) ? ds.backgroundColor : base,
    };
  });
  const fullLabels2 = cleanLabels;
  CHARTS[id] = new Chart(document.getElementById(id), {
    type: "bar",
    data: { labels: cleanLabels, datasets: styledDatasets },
    options: {
      maintainAspectRatio: false,
      plugins: {
        legend: { position: "top", labels: { font: { family: "Tajawal,sans-serif", size: 10, weight: "600" }, boxWidth: 10, boxHeight: 10, padding: 12, usePointStyle: true, pointStyle: "circle" } },
        tooltip: {
          callbacks: { title: (ctx) => sanitizeText(fullLabels2[ctx[0].dataIndex] ?? ctx[0].label, "—") },
        },
      },
      scales: {
